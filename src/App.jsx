  ...
  <style>{`
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    :root {
      ...
      --text:#EDF4FF; --text2:#8AACC4; --text3:#4A6880;
    ...
    /* ── LIGHT THEME ── */
    [data-theme="light"] {
      --ink:#F0F4FA; --ink2:#FFFFFF; --ink3:#E8EEF6;
      --surface:rgba(0,0,0,0.04); --surface2:rgba(0,0,0,0.07);
      --border2:rgba(0,0,0,0.10); --border:rgba(92,224,200,0.30);
      --text:#1A2A3A; --text2:#3D5A72; --text3:#8AACC4;
      --shadow:0 10px 45px rgba(0,0,0,0.12); --shadow-lg:0 24px 80px rgba(0,0,0,0.18);
      --glow-jade:0 0 35px rgba(92,224,200,0.22);
    }
    ...
    /* ── LOADING ── */
    .loading-page{min-height:70vh;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:1.2rem;padding-top:66px}
    ...
    /* ── ERROR STATE ── */
    .error-page{min-height:70vh;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:1rem;text-align:center;padding:2rem;padding-top:66px}
    ...
    /* ── 404 PAGE ── */
    .notfound-page{min-height:100vh;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:1.2rem;text-align:center;padding:2rem;position:relative;overflow:hidden;padding-top:66px}
    ...
    /* ── THEME TOGGLE ── */
    .theme-toggle-btn{position:fixed;bottom:100px;right:26px;z-index:150;width:50px;height:50px;border-radius:50%;background:var(--surface2);border:1.5px solid var(--border2);cursor:pointer;font-size:1.3rem;display:flex;align-items:center;justify-content:center;backdrop-filter:blur(12px);box-shadow:var(--shadow);transition:all 0.3s var(--bounce)}
    .theme-toggle-btn:hover{transform:scale(1.12) rotate(15deg);border-color:var(--jade);box-shadow:var(--glow-jade)}

    /* ── HOORI BOT ── */
    .hoori-btn{position:fixed;bottom:26px;left:26px;z-index:150;cursor:pointer;animation:floatY 4s ease-in-out infinite}
    .hoori-orb{width:56px;height:56px;border-radius:50%;background:radial-gradient(circle at 35% 35%,rgba(168,218,255,0.92),rgba(92,224,200,0.65));border:1.5px solid rgba(92,224,200,0.72);box-shadow:var(--glow-jade),0 0 60px rgba(92,224,200,0.22);display:flex;align-items:center;justify-content:center;position:relative;animation:glowPulse 3s ease-in-out infinite}
    ...
    .hoori-eye.l{left:15px;top:20px}.hoori-eye.r{right:15px;top:20px}
    ...
    .hoori-window{position:fixed;bottom:100px;left:26px;width:340px;max-height:540px;background:var(--ink2);border-radius:var(--r-xl);box-shadow:var(--shadow-lg),var(--glow-jade);border:1px solid var(--border);display:flex;flex-direction:column;overflow:hidden;z-index:160;animation:scaleIn 0.35s var(--bounce)}
    @media(max-width:500px){.hoori-window{width:calc(100vw - 40px);left:20px;right:20px}}
    ...
    /* ── NAVBAR ── */
    .navbar{position:fixed;top:0;left:0;right:0;z-index:300;display:flex;align-items:center;justify-content:space-between;padding:0 2.5rem;height:66px;background:rgba(6,13,24,0.72);backdrop-filter:blur(26px) saturate(1.6);border-bottom:1px solid var(--border2);transition:all 0.4s}
    [data-theme="light"] .navbar{background:rgba(240,244,250,0.82)}
    .navbar.scrolled{background:rgba(6,13,24,0.92);border-bottom-color:rgba(92,224,200,0.2)}
    [data-theme="light"] .navbar.scrolled{background:rgba(240,244,250,0.97)}
    .nav-brand{font-family:var(--serif);font-size:1.5rem;font-weight:700;cursor:pointer;background:linear-gradient(90deg,var(--jade),var(--mist),var(--gold));-webkit-background-clip:text;-webkit-text-fill-color:transparent;letter-spacing:0.12em;text-decoration:none}
    .nav-links{display:flex;gap:1.8rem;align-items:center}
    .nav-link{font-size:0.7rem;font-weight:500;cursor:pointer;text-transform:uppercase;letter-spacing:0.12em;color:var(--text2);transition:color 0.25s;background:none;border:none;font-family:var(--sans);position:relative;text-decoration:none;display:inline-block}
    ...
    @media(max-width:1024px){.nav-links{gap:1rem}.nav-link{font-size:0.65rem}}
    @media(max-width:768px){.nav-links{display:none}.navbar{padding:0 1.25rem}}
    ...
    .wcard-img{position:relative;overflow:hidden;height:220px}
    .wcard-img img{width:100%;height:100%;object-fit:cover;transition:transform 0.7s var(--ease);filter:brightness(0.88) contrast(1.08) saturate(1.1)}
    ...
    .section{padding:5rem 2.5rem;max-width:1300px;margin:0 auto}
    ...
    .hero-bg{position:absolute;inset:0;z-index:0;background:radial-gradient(ellipse at 22% 48%,rgba(92,224,200,0.11) 0%,transparent 52%),radial-gradient(ellipse at 78% 22%,rgba(168,218,255,0.09) 0%,transparent 52%),radial-gradient(ellipse at 50% 85%,rgba(196,170,255,0.07) 0%,transparent 52%),linear-gradient(180deg,var(--ink) 0%,var(--ink2) 55%,var(--ink3) 100%)}
    .hero-city{position:absolute;inset:0;z-index:1;background:linear-gradient(to top,rgba(6,13,24,0.96) 0%,rgba(6,13,24,0.32) 55%,rgba(6,13,24,0.52) 100%),url('https://images.unsplash.com/photo-1538428494232-9c0d8a3ab403?w=1800&q=75') center/cover no-repeat}
    [data-theme="light"] .hero-city{background:linear-gradient(to top,rgba(240,244,250,0.96) 0%,rgba(240,244,250,0.32) 55%,rgba(240,244,250,0.52) 100%),url('https://images.unsplash.com/photo-1538428494232-9c0d8a3ab403?w=1800&q=75') center/cover no-repeat}
    .hero-mist{position:absolute;inset:0;z-index:2;pointer-events:none;background:radial-gradient(ellipse at 50% 65%,rgba(92,224,200,0.055) 0%,transparent 68%);animation:mistFloat 20s ease-in-out infinite}
    .hero-content{position:relative;z-index:5;max-width:920px;padding:2rem}
    .hero-eyebrow{display:inline-flex;align-items:center;gap:0.6rem;padding:0.4rem 1.4rem;background:rgba(92,224,200,0.1);border:1px solid rgba(92,224,200,0.3);border-radius:var(--r-full);font-size:0.7rem;letter-spacing:0.16em;color:var(--jade);margin-bottom:1.8rem;text-transform:uppercase;backdrop-filter:blur(12px);animation:slideUp 0.7s var(--ease) both}
    .hero-title{font-family:var(--serif);font-size:clamp(2.4rem,7vw,5.8rem);font-weight:700;line-height:1.08;margin-bottom:1.4rem;letter-spacing:0.04em;color:var(--text);animation:slideUp 0.7s 0.12s var(--ease) both}
    ...
    .hero-sub{font-size:1rem;color:var(--text2);max-width:560px;margin:0 auto 2.2rem;line-height:1.85;font-family:var(--serif2);font-size:1.15rem;animation:slideUp 0.7s 0.24s var(--ease) both}
    .hero-btns{display:flex;gap:1rem;justify-content:center;flex-wrap:wrap;animation:slideUp 0.7s 0.36s var(--ease) both}
    .hero-stats{display:flex;gap:3rem;justify-content:center;margin-top:3.5rem;flex-wrap:wrap;animation:slideUp 0.7s 0.5s var(--ease) both}
    .hstat-num{font-family:var(--serif);font-size:2.1rem;font-weight:700;color:var(--jade)}
    .hstat-label{font-size:0.66rem;color:var(--text3);letter-spacing:0.12em;text-transform:uppercase;margin-top:3px}

    /* ── SECTION HEADS ── */
    .sec-head{text-align:center;margin-bottom:3.5rem}
    .sec-head h2{font-family:var(--serif);font-size:clamp(1.75rem,4vw,3rem);font-weight:700;color:var(--text);margin-bottom:0.65rem;letter-spacing:0.05em}
    .sec-head p{color:var(--text2);font-size:0.95rem;max-width:480px;margin:0 auto;line-height:1.82;font-family:var(--serif2);font-size:1.05rem}
    .sec-eyebrow{font-family:var(--serif2);font-style:italic;color:var(--gold);font-size:1rem;display:block;margin-bottom:0.45rem;letter-spacing:0.04em}
    ...
    @media(max-width:540px){.srv-grid{grid-template-columns:repeat(2,1fr)}}
    .srv-btn{display:flex;flex-direction:column;align-items:center;gap:0.55rem;padding:1.5rem 0.5rem;border-radius:var(--r-lg);border:1px solid var(--border2);background:var(--surface);backdrop-filter:blur(14px);cursor:pointer;transition:all 0.4s var(--bounce);font-family:var(--sans);font-size:0.72rem;font-weight:600;color:var(--text2);letter-spacing:0.04em}
    .srv-btn:hover{background:rgba(92,224,200,0.1);border-color:var(--jade);transform:translateY(-7px) scale(1.04);color:var(--jade);box-shadow:var(--glow-jade)}
    ...
    /* ── STATS ── */
    .stats-strip{background:linear-gradient(135deg,rgba(92,224,200,0.1),rgba(168,218,255,0.07));border:1px solid rgba(92,224,200,0.2);padding:2.8rem 2.5rem;border-radius:var(--r-xl);display:grid;grid-template-columns:repeat(4,1fr);gap:1rem;text-align:center;backdrop-filter:blur(16px)}
    @media(max-width:680px){.stats-strip{grid-template-columns:repeat(2,1fr)}}
    .ss-num{font-family:var(--serif);font-size:2.3rem;font-weight:700;color:var(--jade)}
    ...
    .testi-av{width:44px;height:44px;border-radius:50%;object-fit:cover;border:1.5px solid rgba(92,224,200,0.5)}
    ...
    /* ── DIRECTORY ── */
    .dir-page{padding-top:66px;min-height:100vh}
    .dir-header{padding:3rem 2.5rem 1.5rem;max-width:1300px;margin:0 auto}
    .dir-controls{display:flex;gap:0.6rem;align-items:center;padding:0.5rem 2.5rem 1rem;max-width:1300px;margin:0 auto;flex-wrap:wrap}
    ...
    .dir-body{display:flex;gap:1.75rem;padding:0.5rem 2.5rem 5rem;max-width:1300px;margin:0 auto}
    .fsidebar{width:265px;flex-shrink:0}
    @media(max-width:960px){.fsidebar{display:none}.dir-body{padding:0.5rem 1.25rem 4rem}}
    ...
    /* ── DETAIL ── */
    .detail-page{padding-top:66px;min-height:100vh}
    .detail-cover{position:relative;height:340px;overflow:hidden}
    ...
    .detail-wrap{max-width:1300px;margin:0 auto;padding:0 2.5rem 5rem}
    .detail-profile{display:flex;gap:2rem;margin-top:-58px;position:relative;z-index:3;align-items:flex-end;flex-wrap:wrap}
    .detail-avatar{width:122px;height:122px;border-radius:var(--r-lg);border:3px solid rgba(92,224,200,0.55);object-fit:cover;box-shadow:var(--glow-jade);filter:brightness(0.92) saturate(1.1)}
    .detail-name{font-family:var(--serif);font-size:2rem;font-weight:700;color:var(--text);letter-spacing:0.04em}
    ...
    @media(max-width:680px){.dstat-grid{grid-template-columns:repeat(2,1fr)}.detail-profile{flex-direction:column}}
    ...
    .port-img{aspect-ratio:1;border-radius:var(--r-md);overflow:hidden;cursor:pointer;position:relative}
    ...
    /* ── CONFIRM ── */
    .confirm-page{padding-top:66px;min-height:100vh;display:flex;align-items:center;justify-content:center}
    .confirm-box{max-width:520px;width:100%;padding:2.5rem;text-align:center}
    .confirm-icon{font-size:4.5rem;margin-bottom:1rem;animation:scaleIn 0.6s var(--bounce)}
    .confirm-title{font-family:var(--serif);font-size:2rem;font-weight:700;color:var(--text);margin-bottom:0.5rem;letter-spacing:0.04em}
    ...
    /* ── AUTH ── */
    .auth-page{min-height:100vh;display:flex;align-items:center;justify-content:center;position:relative;overflow:hidden;padding-top:66px}
    .auth-box{background:rgba(6,13,24,0.93);backdrop-filter:blur(28px);border-radius:var(--r-xl);border:1px solid var(--border);box-shadow:0 32px 90px rgba(0,0,0,0.65),var(--glow-jade);padding:2.6rem;width:100%;max-width:475px;position:relative;z-index:4;margin:2rem;animation:scaleIn 0.4s var(--bounce)}
    [data-theme="light"] .auth-box{background:rgba(255,255,255,0.95)}
    ...
    /* ── DASHBOARD ── */
    .dash-page{padding-top:66px;min-height:100vh}
    .dash-sidebar{width:245px;background:rgba(6,13,24,0.92);backdrop-filter:blur(20px);border-right:1px solid var(--border2);padding:2rem 1rem;position:fixed;top:66px;left:0;bottom:0;overflow-y:auto;z-index:50}
    [data-theme="light"] .dash-sidebar{background:rgba(255,255,255,0.95)}
    @media(max-width:900px){.dash-sidebar{display:none}}
    .dash-main{margin-left:245px;padding:2.5rem}
    @media(max-width:900px){.dash-main{margin-left:0;padding:1.5rem}}
    ...
    .dash-title{font-family:var(--serif);font-size:1.9rem;font-weight:700;color:var(--text);letter-spacing:0.04em}
    ...
    .team-av{width:82px;height:82px;border-radius:50%;object-fit:cover;border:2px solid rgba(92,224,200,0.5);margin:0 auto 0.7rem;display:block;box-shadow:var(--glow-jade)}
    ...
    /* ── FOOTER ── */
    .footer{background:rgba(4,10,20,0.98);border-top:1px solid var(--border2);padding:3.5rem 2.5rem 2rem}
    [data-theme="light"] .footer{background:rgba(26,42,58,0.98)}
    .footer-grid{display:grid;grid-template-columns:2fr 1fr 1fr 1fr;gap:2rem;max-width:1300px;margin:0 auto 2.5rem}
    @media(max-width:720px){.footer-grid{grid-template-columns:1fr 1fr}}
    .footer-brand{font-family:var(--serif);font-size:1.75rem;font-weight:700;background:linear-gradient(90deg,var(--jade),var(--gold));-webkit-background-clip:text;-webkit-text-fill-color:transparent;margin-bottom:0.7rem;letter-spacing:0.1em}
    .footer-desc{font-size:0.8rem;color:var(--text3);line-height:1.82;margin-bottom:1rem}
    .footer-heading{font-weight:800;font-size:0.67rem;color:var(--text2);margin-bottom:0.7rem;text-transform:uppercase;letter-spacing:0.1em}
    .footer-link{display:block;font-size:0.78rem;color:var(--text3);margin-bottom:0.42rem;cursor:pointer;transition:color 0.2s;text-decoration:none}
    .footer-link:hover{color:var(--jade)}

    /* ── CATEGORIES ── */
    .cat-page{padding-top:66px;min-height:100vh}
    ...
    .cat-card{position:relative;overflow:hidden;border-radius:var(--r-xl);cursor:pointer;height:260px}
    ...
    /* ── PORTFOLIO ── */
    .port-page{padding-top:66px;min-height:100vh}
    .port-masonry{columns:3;gap:1rem}
    @media(max-width:900px){.port-masonry{columns:2}}
    @media(max-width:540px){.port-masonry{columns:1}}
    .port-masonry-item{break-inside:avoid;margin-bottom:1rem;border-radius:var(--r-md);overflow:hidden;position:relative;cursor:pointer}
    ...
    .ba-slider{position:relative;overflow:hidden;border-radius:var(--r-lg);height:280px;cursor:col-resize}
    ...
    /* ── EMERGENCY LIVE ── */
    .emrg-live-page{padding-top:66px;min-height:100vh;background:radial-gradient(ellipse at 50% 0%,rgba(255,79,163,0.06),transparent 60%)}
    ...
    .emrg-avatar{width:54px;height:54px;border-radius:50%;object-fit:cover;border:2px solid rgba(255,126,182,0.5);filter:brightness(0.92)}
    ...
    /* ── SMART CITY ── */
    .smart-page{padding-top:66px;min-height:100vh}
    ...
    /* ── TOAST ── */
    .toast-wrap{position:fixed;bottom:1.8rem;right:1.8rem;z-index:700;display:flex;flex-direction:column;gap:0.5rem}
    ...
    /* ── EMERGENCY FLOAT ── */
    .emrg-float{position:fixed;bottom:26px;right:26px;z-index:150;width:58px;height:58px;border-radius:50%;background:linear-gradient(135deg,rgba(255,126,182,0.28),rgba(196,170,255,0.22));border:1.5px solid rgba(255,126,182,0.52);display:flex;align-items:center;justify-content:center;cursor:pointer;font-size:1.45rem;box-shadow:var(--glow-rose);animation:glowRose 2.5s ease-in-out infinite,floatY2 5.5s ease-in-out infinite;transition:transform 0.3s var(--bounce)}
...
const ThemeProvider = ({ children }) => {
  const [theme, setTheme] = useState(() => localStorage.getItem('srv_theme') || 'dark');
  useEffect(() => {
    localStorage.setItem('srv_theme', theme);
...
const PROFESSION_IMAGES = {
  Electrician:{cover:'https://images.unsplash.com/photo-1621905251189-08b45d6a269e?w=900&q=80',avatar:'https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=300&q=80',portfolio:['https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=400&q=80','https://images.unsplash.com/photo-1504328345606-18bbc8c9d7d1?w=400&q=80','https://images.unsplash.com/photo-1621905251918-bf1b7a4e6fe2?w=400&q=80','https://images.unsplash.com/photo-1497366216548-37526070297c?w=400&q=80'],categoryImg:'https://images.unsplash.com/photo-1621905251189-08b45d6a269e?w=600&q=80'},
  Plumber:{cover:'https://images.unsplash.com/photo-1607472586893-edb57bdc0e39?w=900&q=80',avatar:'https://images.unsplash.com/photo-1607472586893-edb57bdc0e39?w=300&q=80',portfolio:['https://images.unsplash.com/photo-1607472586893-edb57bdc0e39?w=400&q=80','https://images.unsplash.com/photo-1585771724684-38269d6639fd?w=400&q=80','https://images.unsplash.com/photo-1558618047-3c8c76ca7d13?w=400&q=80','https://images.unsplash.com/photo-1504328345606-18bbc8c9d7d1?w=400&q=80'],categoryImg:'https://images.unsplash.com/photo-1607472586893-edb57bdc0e39?w=600&q=80'},
  Cleaner:{cover:'https://images.unsplash.com/photo-1581578731548-c64695cc6952?w=900&q=80',avatar:'https://images.unsplash.com/photo-1581578731548-c64695cc6952?w=300&q=80',portfolio:['https://images.unsplash.com/photo-1581578731548-c64695cc6952?w=400&q=80','https://images.unsplash.com/photo-1527515545081-5db817172677?w=400&q=80','https://images.unsplash.com/photo-1563453392212-326f5e854473?w=400&q=80','https://images.unsplash.com/photo-1556909114-f6e7ad7d3136?w=400&q=80'],categoryImg:'https://images.unsplash.com/photo-1581578731548-c64695cc6952?w=600&q=80'},
  'AC Technician':{cover:'https://images.unsplash.com/photo-1473341304170-971dccb5ac1e?w=900&q=80',avatar:'https://images.unsplash.com/photo-1504328345606-18bbc8c9d7d1?w=300&q=80',portfolio:['https://images.unsplash.com/photo-1473341304170-971dccb5ac1e?w=400&q=80','https://images.unsplash.com/photo-1585771724684-38269d6639fd?w=400&q=80','https://images.unsplash.com/photo-1621905251189-08b45d6a269e?w=400&q=80','https://images.unsplash.com/photo-1497366216548-37526070297c?w=400&q=80'],categoryImg:'https://images.unsplash.com/photo-1473341304170-971dccb5ac1e?w=600&q=80'},
  Carpenter:{cover:'https://images.unsplash.com/photo-1558618047-3c8c76ca7d13?w=900&q=80',avatar:'https://images.unsplash.com/photo-1558618047-3c8c76ca7d13?w=300&q=80',portfolio:['https://images.unsplash.com/photo-1558618047-3c8c76ca7d13?w=400&q=80','https://images.unsplash.com/photo-1541123437800-1bb1317badc2?w=400&q=80','https://images.unsplash.com/photo-1504328345606-18bbc8c9d7d1?w=400&q=80','https://images.unsplash.com/photo-1555041469-a586c61ea9bc?w=400&q=80'],categoryImg:'https://images.unsplash.com/photo-1558618047-3c8c76ca7d13?w=600&q=80'},
  Painter:{cover:'https://images.unsplash.com/photo-1562259929-b4e1fd3aef09?w=900&q=80',avatar:'https://images.unsplash.com/photo-1562259929-b4e1fd3aef09?w=300&q=80',portfolio:['https://images.unsplash.com/photo-1562259929-b4e1fd3aef09?w=400&q=80','https://images.unsplash.com/photo-1589939705384-5185137a7f0f?w=400&q=80','https://images.unsplash.com/photo-1503387762-592deb58ef4e?w=400&q=80','https://images.unsplash.com/photo-1513694203232-719a280e022f?w=400&q=80'],categoryImg:'https://images.unsplash.com/photo-1562259929-b4e1fd3aef09?w=600&q=80'},
  Tutor:{cover:'https://images.unsplash.com/photo-1522202176988-66273c2fd55f?w=900&q=80',avatar:'https://images.unsplash.com/photo-1522202176988-66273c2fd55f?w=300&q=80',portfolio:['https://images.unsplash.com/photo-1522202176988-66273c2fd55f?w=400&q=80','https://images.unsplash.com/photo-1503676260728-1c00da094a0b?w=400&q=80','https://images.unsplash.com/photo-1456513080510-7bf3a84b82f8?w=400&q=80','https://images.unsplash.com/photo-1488190211105-8b0e65b80b4e?w=400&q=80'],categoryImg:'https://images.unsplash.com/photo-1522202176988-66273c2fd55f?w=600&q=80'},
  Mechanic:{cover:'https://images.unsplash.com/photo-1625047509168-a7026f36de04?w=900&q=80',avatar:'https://images.unsplash.com/photo-1625047509168-a7026f36de04?w=300&q=80',portfolio:['https://images.unsplash.com/photo-1625047509168-a7026f36de04?w=400&q=80','https://images.unsplash.com/photo-1487754180451-c456f719a1fc?w=400&q=80','https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=400&q=80','https://images.unsplash.com/photo-1504328345606-18bbc8c9d7d1?w=400&q=80'],categoryImg:'https://images.unsplash.com/photo-1625047509168-a7026f36de04?w=600&q=80'},
  Beautician:{cover:'https://images.unsplash.com/photo-1560066984-138dadb4c035?w=900&q=80',avatar:'https://images.unsplash.com/photo-1560066984-138dadb4c035?w=300&q=80',portfolio:['https://images.unsplash.com/photo-1560066984-138dadb4c035?w=400&q=80','https://images.unsplash.com/photo-1487412947147-5cebf100d3c6?w=400&q=80','https://images.unsplash.com/photo-1521590832167-7bcbfaa6381f?w=400&q=80','https://images.unsplash.com/photo-1522337360788-8b13dee7a37e?w=400&q=80'],categoryImg:'https://images.unsplash.com/photo-1560066984-138dadb4c035?w=600&q=80'},
  Chef:{cover:'https://images.unsplash.com/photo-1577219491135-ce391730fb2c?w=900&q=80',avatar:'https://images.unsplash.com/photo-1577219491135-ce391730fb2c?w=300&q=80',portfolio:['https://images.unsplash.com/photo-1577219491135-ce391730fb2c?w=400&q=80','https://images.unsplash.com/photo-1547592180-85f173990554?w=400&q=80','https://images.unsplash.com/photo-1514986888952-8cd320577b68?w=400&q=80','https://images.unsplash.com/photo-1504674900247-0877df9cc836?w=400&q=80'],categoryImg:'https://images.unsplash.com/photo-1577219491135-ce391730fb2c?w=600&q=80'},
  Driver:{cover:'https://images.unsplash.com/photo-1449965408869-eaa3f722e40d?w=900&q=80',avatar:'https://images.unsplash.com/photo-1449965408869-eaa3f722e40d?w=300&q=80',portfolio:['https://images.unsplash.com/photo-1449965408869-eaa3f722e40d?w=400&q=80','https://images.unsplash.com/photo-1494976388531-d1058494cdd8?w=400&q=80','https://images.unsplash.com/photo-1568605117036-5fe5e7bab0b7?w=400&q=80','https://images.unsplash.com/photo-1484185759689-8f7f0c03e352?w=400&q=80'],categoryImg:'https://images.unsplash.com/photo-1449965408869-eaa3f722e40d?w=600&q=80'},
  Gardener:{cover:'https://images.unsplash.com/photo-1416879595882-3373a0480b5b?w=900&q=80',avatar:'https://images.unsplash.com/photo-1416879595882-3373a0480b5b?w=300&q=80',portfolio:['https://images.unsplash.com/photo-1416879595882-3373a0480b5b?w=400&q=80','https://images.unsplash.com/photo-1585320806297-9794b3e4aaae?w=400&q=80','https://images.unsplash.com/photo-1523348837708-15d4a09cfac2?w=400&q=80','https://images.unsplash.com/photo-1463936575829-25148e1db1b8?w=400&q=80'],categoryImg:'https://images.unsplash.com/photo-1416879595882-3373a0480b5b?w=600&q=80'},
};
...
const M_NAMES=['Ali Khan','Bilal Ahmed','Usman Ali','Omar Shafiq','Hamza Butt','Tariq Mehmood','Imran Rao','Asad Chaudhry','Kamran Dar','ali ahmed q','Ahmed Siddiqui','Hassan Raza'];
...
function getWorkerImages(skill,isFem,idx){
  const prof=PROFESSION_IMAGES[skill]||PROFESSION_IMAGES['Electrician'];
  const avatar=isFem?`https://randomuser.me/api/portraits/women/${(idx%60)+1}.jpg`:`https://randomuser.me/api/portraits/men/${(idx%60)+1}.jpg`;
  return{avatar,cover:prof.cover,portfolio:prof.portfolio};
}
function genWorkers(){
  return Array.from({length:105},(_,i)=>{
    ...
    const imgs=getWorkerImages(skill,isFem,i);
...
/* ═══════════════════════════════════════════════════
   LOADING SPINNER
═══════════════════════════════════════════════════ */
const LoadingSpinner = ({message='Loading...',sub='Please wait while we prepare things ✨'}) => (
...
const SkeletonCard = () => (
  <div className="sk-card card">
    ...
    <div className="sk-body">
      ...
      <div className="skeleton sk-line w60" style={{height:32,borderRadius:99}}/>
    </div>
  </div>
);
...
/* ═══════════════════════════════════════════════════
   HOORI BOT — Full Working Chatbot
═══════════════════════════════════════════════════ */
const HOORI_RESPONSES = [
  {keys:['hello','hi','hey','salam','assalam'],reply:'Hello! 👋 Welcome to Servire! I\'m Hoori, your AI assistant. How can I help you today?'},
  {keys:['book','booking','appointment','hire'],reply:'📅 To book a worker:\n1. Browse Workers page\n2. Click on a profile\n3. Click "Book Now"\n4. Select date & time\n5. Confirm! Easy as that ✨',nav:{label:'Browse Workers →',to:'/workers'}},
  {keys:['electrician','electric','wiring','power'],reply:'⚡ We have verified electricians available in all major cities. Average rating: 4.8 ⭐',nav:{label:'Find Electricians →',to:'/workers'}},
  {keys:['plumber','plumbing','water','leak','pipe'],reply:'🔧 Our plumbers are available 24/7 including emergency services. Fast response guaranteed!',nav:{label:'Find Plumbers →',to:'/workers'}},
  {keys:['clean','cleaner','cleaning','maid','housekeeping'],reply:'🧹 Our cleaning professionals provide deep cleaning, regular maintenance, and office cleaning services.',nav:{label:'Find Cleaners →',to:'/workers'}},
  {keys:['ac','air condition','cooling','hvac'],reply:'❄️ AC technicians for installation, repair, and maintenance. Same-day service available!',nav:{label:'Find AC Technicians →',to:'/workers'}},
  {keys:['emergency','urgent','help','sos','immediate'],reply:'🚨 Emergency services available 24/7! Our rapid response team can reach you within minutes.',nav:{label:'Emergency Page →',to:'/emergency'}},
  {keys:['price','cost','rate','fee','charge','rate','how much'],reply:'💰 Service prices range from Rs. 500 to Rs. 5,000+ depending on the service. All prices are displayed transparently on worker profiles. No hidden fees!'},
  {keys:['verified','safe','trust','background','check','secure'],reply:'✅ All Servire professionals undergo:\n• Government ID verification\n• Skill assessment tests\n• Background screening\n• Reference checks\nYour safety is our #1 priority!'},
  {keys:['category','categories','type','service type','kind'],reply:'🗂 We offer 12 service categories including Electrician, Plumber, Cleaner, AC Tech, Carpenter, Painter, Tutor, Mechanic, Beautician, Chef, Driver, and Gardener.',nav:{label:'Browse Categories →',to:'/categories'}},
  {keys:['portfolio','work','gallery','photo','sample'],reply:'📸 View real work samples from our verified professionals in our portfolio gallery.',nav:{label:'View Portfolio →',to:'/portfolio'}},
  {keys:['about','servire','platform','company','what is'],reply:'🌊 Servire is Pakistan\'s trusted platform connecting verified home service professionals with families since 2021. 105+ workers, 10+ cities, 1,240+ jobs completed!',nav:{label:'About Us →',to:'/about'}},
  {keys:['contact','support','help desk','question','problem'],reply:'📞 Need support? Our team is available Mon-Sat 9AM-8PM. Emergency support 24/7.',nav:{label:'Contact Us →',to:'/contact'}},
  {keys:['login','sign in','signin','log in'],reply:'🔐 Sign in to access your dashboard, bookings, and saved workers.',nav:{label:'Login →',to:'/login'}},
  {keys:['signup','register','create','join','new account'],reply:'✨ Join Servire in under 2 minutes! Create your account as a User or Service Worker.',nav:{label:'Sign Up →',to:'/signup'}},
  {keys:['dashboard','my account','profile','booking history'],reply:'👤 Your dashboard shows bookings, saved workers, and account settings.',nav:{label:'My Dashboard →',to:'/dashboard'}},
  {keys:['city','cities','lahore','karachi','islamabad','gujrat','faisalabad','multan'],reply:'📍 We\'re in 10+ cities: Gujrat, Lahore, Karachi, Islamabad, Rawalpindi, Faisalabad, Multan, Peshawar, Sialkot, Quetta — and expanding!'},
  {keys:['cancel','refund','reschedule'],reply:'🔄 You can cancel or reschedule bookings up to 2 hours before the scheduled time at no charge. Contact support for refunds within 24 hours of service.'},
  {keys:['payment','pay','jazzcash','easypaisa','cash'],reply:'💳 Payment options: Cash, JazzCash, and Easypaisa. Payment is made directly to the professional after service completion.'},
  {keys:['rating','review','stars','feedback'],reply:'⭐ All reviews on Servire are from verified bookings only. You can rate workers after each completed service to maintain quality.'},
  {keys:['female','women','lady','girl'],reply:'👩 Yes! We have verified female professionals for families who prefer same-gender service providers. Filter by "female worker" in the Workers page.',nav:{label:'Browse Workers →',to:'/workers'}},
  {keys:['thanks','thank you','shukriya','great','awesome'],reply:'You\'re most welcome! 🌊 Anything else I can help you with? Servire is here for all your home service needs!'},
  {keys:['bye','goodbye','exit','close'],reply:'Goodbye! 👋 Come back anytime you need a trusted professional. Servire is always here for you! 🌊'},
];
const HOORI_DEFAULT = '🤔 I didn\'t quite catch that. Try asking about:\n• How to book a worker\n• Service categories\n• Pricing & payments\n• Emergency help\n• Account & login\n\nOr tap a quick suggestion below! 💡';
const HOORI_CHIPS = ['🔍 Find Workers','📅 How to Book?','🚨 Emergency Help','💰 Pricing Info','✅ Are Workers Verified?','📍 Which Cities?'];

const HooriBot = () => {
  const navigate = useNavigate();
  const {addToast} = useApp();
  const [open,setOpen] = useState(false);
  const [msgs,setMsgs] = useState([{role:'bot',text:'🌊 Hi! I\'m Hoori, your Servire assistant!\n\nI can help you find workers, answer questions, or navigate the platform. What can I do for you?'}]);
  ...
  const send = async (text) => {
    ...
    if(res){
      setMsgs(p=>[...p,{role:'bot',text:res.reply,nav:res.nav||null}]);
    } else {
      setMsgs(p=>[...p,{role:'bot',text:HOORI_DEFAULT}]);
    }
    setTyping(false);
  };
      ...
      <div className="hoori-window">
        ...
        <div className="hoori-head">
          <div className="hoori-head-info">
            ...
            <div>
              <div className="hoori-head-name">Hoori Assistant</div>
              <div className="hoori-head-status">● Online · Instant replies</div>
            </div>
          </div>
          ...
          {msgs.map((m,i)=>(
            <div key={i} className={`h-msg ${m.role}`}>
              ...
              <div className="h-msg-bubble">
                ...
                {m.nav&&<button className="h-nav-btn" onClick={()=>handleNav(m.nav.to)}>🔗 {m.nav.label}</button>}
              </div>
            </div>
          ))}
          {typing&&(
            <div className="h-msg bot">
              <div className="h-msg-av">🌊</div>
              <div className="h-typing"><span/><span/><span/></div>
            </div>
          )}
          <div ref={scrollRef}/>
        </div>
        ...
        <div className="hoori-chips">
          {HOORI_CHIPS.map((c,i)=>(
            <button key={i} className="h-chip" onClick={()=>handleChip(c)}>{c}</button>
          ))}
        </div>
...
const Navbar=()=>{
  ...
  const NAV_ITEMS=[['/', 'Home'],['/workers','Workers'],['/categories','Categories'],['/portfolio','Portfolio'],['/emergency','Emergency'],['/smartcity','Smart City'],['/about','About'],['/contact','Contact']];
  return(
    <nav className={`navbar ${scrolled?'scrolled':''}`}>
      <Link className="nav-brand" to="/">✦ SERVIRE</Link>
      ...
      <div className="nav-actions">
        <Link className="btn btn-icon" style={{position:'relative',textDecoration:'none'}} to="/favorites" title="Favorites">
          💖
          {favorites.length>0&&<span style={{position:'absolute',top:-4,right:-4,background:'var(--jade)',color:'var(--ink)',borderRadius:99,fontSize:'0.55rem',padding:'1px 5px',fontWeight:800}}>{favorites.length}</span>}
        </Link>
        {user?(
          <Link className="btn btn-ghost btn-sm" to="/dashboard">👤 {user.name.split(' ')[0]}</Link>
        ):(<>
          <Link className="btn btn-ghost btn-sm" to="/login">Login</Link>
          <Link className="btn btn-jade btn-sm" to="/signup">Sign Up ✦</Link>
        </>)}
      </div>
    </nav>
  );
  ...
  return(
    <footer className="footer">
      <div className="footer-grid">
        <div>
          <div className="footer-brand">✦ SERVIRE</div>
          <p className="footer-desc">Connecting verified professionals with families across Pakistan. Your comfort, our craft — since 2021.</p>
          <div style={{display:'flex',gap:'0.75rem',marginTop:'0.75rem'}}>
            {['📘','🐦','📸','💬'].map((ic,i)=>(
              <span key={i} style={{cursor:'pointer',fontSize:'1.2rem',opacity:0.5,transition:'opacity 0.2s'}} onMouseEnter={e=>e.target.style.opacity=1} onMouseLeave={e=>e.target.style.opacity=0.5}>{ic}</span>
            ))}
          </div>
        </div>
        <div>
          <div className="footer-heading">Pages</div>
          {[['/workers','Workers'],['/categories','Categories'],['/portfolio','Portfolio'],['/emergency','Emergency'],['/smartcity','Smart City']].map(([to,l])=>(
            <Link key={to} className="footer-link" to={to}>{l}</Link>
          ))}
        </div>
        <div>
          <div className="footer-heading">Company</div>
          {[['/about','About Us'],['/contact','Contact'],['/admin','Admin']].map(([to,l])=>(
            <Link key={to} className="footer-link" to={to}>{l}</Link>
          ))}
        </div>
        <div>
          <div className="footer-heading">Support</div>
          {['FAQs','Privacy Policy','Terms of Service','Safety Tips','Careers'].map(s=><span key={s} className="footer-link" style={{cursor:'pointer'}}>{s}</span>)}
        </div>
      </div>
      <div style={{borderTop:'1px solid var(--border2)',paddingTop:'1.5rem',maxWidth:1300,margin:'0 auto',display:'flex',justifyContent:'space-between',alignItems:'center',fontSize:'0.73rem',color:'var(--text3)',flexWrap:'wrap',gap:'0.5rem'}}>
        <span>© 2025 Servire — Crafted with 🌊 in Pakistan</span>
        <span style={{color:'var(--jade)',letterSpacing:'0.1em',fontSize:'0.68rem'}}>VERIFIED · TRUSTED · LOVED</span>
      </div>
    </footer>
  );
};
...
const WorkerCard=({worker:w,onClick})=>{
  ...
  const heart=e=>{e.stopPropagation();const added=toggleFavorite(w.id);addToast(added?`Saved ${w.name.split(' ')[0]}! 💖`:'Removed from saved','');};
  ...
  return(
    <div className="card wcard" onClick={handleClick}>
      <div className="wcard-img">
        <img src={w.avatar} alt={w.name} onError={e=>{e.target.onerror=null;e.target.src=profImg.avatar;}}/>
...
const BeforeAfterSlider=({before,after})=>{
  ...
  const ref=useRef();
  const drag=useCallback(e=>{if(!ref.current)return;const r=ref.current.getBoundingClientRect();const x=(e.touches?e.touches[0].clientX:e.clientX)-r.left;setPos(Math.max(5,Math.min(95,(x/r.width)*100)));},[]);
  return(
    <div ref={ref} className="ba-slider" onMouseMove={drag} onTouchMove={drag}>
      <div className="ba-after"><img src={after} alt="After" style={{filter:'brightness(0.9) saturate(1.1)'}}/></div>
      <div className="ba-before" style={{clipPath:`inset(0 ${100-pos}% 0 0)`}}><img src={before} alt="Before" style={{filter:'brightness(0.75) saturate(0.7) grayscale(0.3)'}}/></div>
      <div className="ba-divider" style={{left:`${pos}%`}}><div className="ba-handle">⇔</div></div>
      <div style={{position:'absolute',top:12,left:16,background:'rgba(6,13,24,0.8)',color:'var(--text3)',fontSize:'0.65rem',fontWeight:700,padding:'2px 8px',borderRadius:99,backdropFilter:'blur(8px)'}}>BEFORE</div>
      <div style={{position:'absolute',top:12,right:16,background:'rgba(92,224,200,0.2)',color:'var(--jade)',fontSize:'0.65rem',fontWeight:700,padding:'2px 8px',borderRadius:99,backdropFilter:'blur(8px)',border:'1px solid rgba(92,224,200,0.4)'}}>AFTER</div>
    </div>
  );
};
        ...
        <div className="modal-body">
          ...
          {phase==='scanning'&&(
            <div style={{textAlign:'center',padding:'1rem 0'}}>
              ...
              <p style={{fontSize:'0.8rem',color:'var(--text2)'}}>Finding available {sel} professionals</p>
            </div>
          )}
          {phase==='found'&&(<>
            ...
            {NEARBY_WORKERS.map(w=>(
              <div key={w.name} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'0.85rem 1rem',marginBottom:'0.5rem',background:'var(--surface)',border:'1px solid var(--border2)',borderRadius:'var(--r-md)'}}>
                <div style={{display:'flex',alignItems:'center',gap:'0.7rem'}}><img src={w.img} alt={w.name} style={{width:40,height:40,borderRadius:'50%',objectFit:'cover',border:'1.5px solid rgba(255,126,182,0.5)'}}/><div><div style={{fontWeight:700,fontSize:'0.85rem',color:'var(--text)'}}>{w.name}</div><div style={{fontSize:'0.72rem',color:'var(--text3)'}}>{w.skill}</div></div></div>
  ...
  return(
    <PageWrapper>
      ...
      <div className="section">
        <div className="sec-head"><div className="sec-divider">✦</div><span className="sec-eyebrow">Real Professionals, Real Work</span><h2>Profession Showcase</h2><p>See authentic work from our verified professionals — every image tells a real story.</p></div>
        <div style={{display:'grid',gridTemplateColumns:'2fr 1fr 1fr',gridTemplateRows:'200px 200px',gap:'0.75rem',borderRadius:'var(--r-xl)',overflow:'hidden'}}>
          {[{img:'https://images.unsplash.com/photo-1621905251189-08b45d6a269e?w=900&q=80',label:'Electrician',style:{gridRow:'1/3'}},{img:'https://images.unsplash.com/photo-1581578731548-c64695cc6952?w=600&q=80',label:'Cleaning'},{img:'https://images.unsplash.com/photo-1558618047-3c8c76ca7d13?w=600&q=80',label:'Carpentry'},{img:'https://images.unsplash.com/photo-1577219491135-ce391730fb2c?w=600&q=80',label:'Chef Services'},{img:'https://images.unsplash.com/photo-1607472586893-edb57bdc0e39?w=600&q=80',label:'Plumbing'}].map((item,i)=>(
            <div key={i} style={{position:'relative',overflow:'hidden',cursor:'pointer',...item.style}} onClick={()=>navigate('/categories')}>
              <img src={item.img} alt={item.label} style={{width:'100%',height:'100%',objectFit:'cover',filter:'brightness(0.72) saturate(1.15)',transition:'transform 0.6s var(--ease)'}} onMouseEnter={e=>{e.currentTarget.style.transform='scale(1.06)';e.currentTarget.style.filter='brightness(0.88) saturate(1.25)'}} onMouseLeave={e=>{e.currentTarget.style.transform='scale(1)';e.currentTarget.style.filter='brightness(0.72) saturate(1.15)'}}/>
              <div style={{position:'absolute',inset:0,background:'linear-gradient(to top,rgba(6,13,24,0.75),transparent)'}}/>
              <div style={{position:'absolute',bottom:12,left:14,fontFamily:'var(--serif)',color:'var(--text)',fontSize:'0.85rem',fontWeight:700,letterSpacing:'0.05em'}}>{item.label}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="section" style={{paddingTop:0}}>
        <div className="sec-head"><div className="sec-divider">✦</div><span className="sec-eyebrow">What can we help with?</span><h2>Our Services</h2><p>From emergency repairs to everyday home maintenance — your complete service solution.</p></div>
      ...
      {/* HOW IT WORKS */}
      <div style={{background:'rgba(92,224,200,0.03)',borderTop:'1px solid var(--border2)',borderBottom:'1px solid var(--border2)',padding:'5rem 2.5rem'}}>
      ...
      <div style={{padding:'4rem 2.5rem'}}>
        <div className="stats-strip" style={{maxWidth:1300,margin:'0 auto'}}>
        ...
        <div className="g2">
          {TESTIMONIALS.map((t,i)=>(
            <div key={i} className="testi card" style={{opacity:i===tiIdx||i===(tiIdx+1)%4?1:0.52,transform:i===tiIdx?'scale(1.025)':'scale(1)',transition:'all 0.5s var(--ease)'}}>
              ...
              <div className="flex items-center gap-3"><img src={t.av} alt={t.name} className="testi-av"/><div><div className="testi-name">{t.name}</div><div className="testi-loc">📍 {t.loc}</div></div></div>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* NEWSLETTER */}
      <div style={{maxWidth:1300,margin:'0 auto 4rem',padding:'0 2.5rem'}}>
        <div style={{background:'linear-gradient(135deg,rgba(92,224,200,0.1),rgba(168,218,255,0.07))',border:'1px solid rgba(92,224,200,0.22)',borderRadius:'var(--r-xl)',padding:'3.5rem 2.5rem',textAlign:'center',backdropFilter:'blur(16px)'}}>
          <h2 style={{fontFamily:'var(--serif)',fontSize:'2rem',fontWeight:700,color:'var(--text)',marginBottom:'0.5rem',letterSpacing:'0.05em'}}>Stay in the Current 🌊</h2>
    ...
    <PageWrapper>
      <div className="dir-page">
        <div className="dir-header">
          ...
          <h1 style={{fontFamily:'var(--serif)',fontSize:'2.2rem',fontWeight:700,letterSpacing:'0.04em',color:'var(--text)',marginBottom:'0.2rem',marginTop:'0.5rem'}}>Find Your Professional 🌊</h1>
        ...
        <div className="dir-controls">
          <div className="search-wrap"><span className="search-ico">🔍</span><input className="input" value={search} onChange={e=>{setSearch(e.target.value);setCp(1);}} placeholder="Search by name or skill..."/></div>
      ...
      <div className="confirm-box card">
        ...
        <div className="confirm-detail">
          {[['Booking ID',bkId],['Worker',`${worker.name} (${worker.skill})`],['Date',booking.date],['Time',booking.time],['Address',booking.addr],['Total',`Rs. ${booking.total.toLocaleString()}`],['Type',booking.emerg?'🚨 Emergency':'✦ Standard']].map(([l,v])=>(
            <div key={l} className="confirm-row"><span style={{color:'var(--text3)'}}>{l}</span><span style={{fontWeight:700,color:'var(--text)'}}>{v}</span></div>
          ))}
        </div>
    ...
    <PageWrapper>
      ...
      <div className="detail-page">
        <div className="detail-cover">
          <img src={profImgs.cover} alt="cover" onError={e=>{e.target.onerror=null;e.target.src='https://images.unsplash.com/photo-1504328345606-18bbc8c9d7d1?w=900&q=80';}}/>
        ...
        <div className="detail-wrap">
          ...
          <div className="detail-profile">
            <img src={worker.avatar} alt={worker.name} className="detail-avatar" onError={e=>{e.target.onerror=null;e.target.src=profImgs.avatar;}}/>
            <div style={{flex:1}}>
          ...
          <div className="g2" style={{gap:'2.5rem',alignItems:'start'}}>
            <div>
            ...
            <div>
              ...
              <div className="port-grid">
                {profImgs.portfolio.map((img,pi)=>(<div key={pi} className="port-img"><img src={img} alt={`Work ${pi+1}`} onError={e=>{e.target.onerror=null;e.target.src='https://images.unsplash.com/photo-1504328345606-18bbc8c9d7d1?w=400&q=80';}}/><div className="port-img-overlay"/></div>))}
              </div>
          ...
          <div className="sec-title">Customer Reviews ({worker.totalReviews})</div>
          {worker.reviews.map(r=>(
            <div key={r.id} className="rev-card card">
      ...
      <div className="cat-page">
        ...
        <div className="section">
          <div className="cat-hero-grid">
            {cats.map(c=>(
              <div key={c.skill} className="cat-card" onClick={()=>navigate('/workers')}>
                <img src={c.img} alt={c.skill} onError={e=>{e.target.onerror=null;e.target.src='https://images.unsplash.com/photo-1504328345606-18bbc8c9d7d1?w=600&q=80';}}/>
          ...
          <div style={{marginTop:'5rem'}}>
            <div className="sec-head"><div className="sec-divider">✦</div><span className="sec-eyebrow">Every category includes</span><h2>What You Can Expect</h2></div>
            <div className="g3">
              {[{ic:'📸',t:'Verified Portfolios',d:'Real photos of actual work done — no stock images.'},{ic:'✦',t:'Credential Checks',d:'ID verification and background screening.'},{ic:'⭐',t:'Genuine Reviews',d:'All ratings from verified bookings only.'},{ic:'💰',t:'Transparent Pricing',d:'No hidden fees — what you see is what you pay.'},{ic:'🚨',t:'Emergency Support',d:'Many pros available for same-day emergency bookings.'},{ic:'🤝',t:'Satisfaction Guarantee',d:'Our support steps in within 24 hours if needed.'}].map(f=>(
                <div key={f.t} className="card" style={{padding:'1.8rem'}}>
                  <div style={{fontSize:'2rem',marginBottom:'0.7rem'}}>{f.ic}</div>
                  <div style={{fontFamily:'var(--serif)',fontSize:'0.95rem',fontWeight:700,color:'var(--text)',marginBottom:'0.45rem'}}>{f.t}</div>
                  <div style={{fontSize:'0.82rem',color:'var(--text2)',lineHeight:1.72}}>{f.d}</div>
                </div>
              ))}
            </div>
          </div>
    ...
    <PageWrapper>
      <div className="port-page">
        ...
        <div className="section">
          ...
          <div className="port-masonry">
            {allImages.filter(a=>filter==='All'||a.skill===filter).map((item,i)=>(
              <div key={i} className="port-masonry-item" onClick={()=>navigate('/workers')}>
                <img src={item.img} alt={item.label} onError={e=>{e.target.onerror=null;e.target.src='https://images.unsplash.com/photo-1504328345606-18bbc8c9d7d1?w=400&q=80';}}/>
          ...
          <div style={{marginTop:'5rem'}}>
            <div className="sec-head"><div className="sec-divider">✦</div><span className="sec-eyebrow">See the difference</span><h2>Before & After</h2><p>Drag the slider to compare work quality from our certified professionals.</p></div>
            <div className="g2" style={{alignItems:'start',gap:'2rem'}}>
              ...
              <div><BeforeAfterSlider before="https://images.unsplash.com/photo-1581578731548-c64695cc6952?w=600&q=60" after="https://images.unsplash.com/photo-1527515545081-5db817172677?w=600&q=80"/><div style={{textAlign:'center',marginTop:'0.75rem',fontSize:'0.78rem',color:'var(--text3)'}}>Home Cleaning — Before & After</div></div>
            </div>
          </div>
          <div style={{marginTop:'5rem'}}>
            <div className="sec-head"><div className="sec-divider">✦</div><span className="sec-eyebrow">Portfolio highlights</span><h2>Top Workers with Rich Portfolios</h2></div>
            <div className="g3">
              {topWorkers.slice(0,6).map(w=>{const profImgs=PROFESSION_IMAGES[w.skill]||PROFESSION_IMAGES['Electrician'];return(
                <div key={w.id} className="card" style={{cursor:'pointer',overflow:'hidden'}} onClick={()=>navigate(`/worker/${w.id}`)}>
                  <div style={{position:'relative',height:180,overflow:'hidden'}}>
                    <img src={profImgs.cover} alt={w.skill} style={{width:'100%',height:'100%',objectFit:'cover',filter:'brightness(0.7) saturate(1.1)',transition:'transform 0.6s var(--ease)'}} onMouseEnter={e=>e.currentTarget.style.transform='scale(1.06)'} onMouseLeave={e=>e.currentTarget.style.transform='scale(1)'}/>
                    <div style={{position:'absolute',inset:0,background:'linear-gradient(to top,rgba(6,13,24,0.85),transparent)'}}/>
                    <img src={w.avatar} alt={w.name} style={{position:'absolute',bottom:-20,left:16,width:50,height:50,borderRadius:'50%',objectFit:'cover',border:'2px solid rgba(92,224,200,0.6)',filter:'brightness(0.92)'}}/>
                  </div>
                  <div style={{padding:'1.5rem 1rem 1rem'}}>
                    <div style={{fontFamily:'var(--serif2)',fontWeight:700,fontSize:'1rem',color:'var(--text)',marginBottom:'0.2rem'}}>{w.name}</div>
                    <div style={{fontSize:'0.7rem',color:'var(--jade)',fontWeight:700,marginBottom:'0.7rem'}}>{w.skill} · ⭐ {w.rating}</div>
                    <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'3px'}}>{profImgs.portfolio.slice(0,3).map((img,pi)=>(<div key={pi} style={{aspectRatio:'1',borderRadius:'var(--r-sm)',overflow:'hidden'}}><img src={img} alt="" style={{width:'100%',height:'100%',objectFit:'cover',filter:'brightness(0.85)'}}/></div>))}</div>
                  </div>
                </div>
              );})}
            </div>
...
const EmergencyPage=()=>{
  ...
  const activate=cat=>{setActiveEmrg(cat);setScanning(true);setFound(false);setTimeout(()=>{setScanning(false);setFound(true);},3500);};
  return(
    <PageWrapper>
      <div className="emrg-live-page">
        <div style={{position:'relative',overflow:'hidden'}}>
          <div style={{height:'40vh',background:'linear-gradient(to bottom,rgba(255,79,163,0.08),transparent)',borderBottom:'1px solid rgba(255,79,163,0.15)',display:'flex',alignItems:'center',justifyContent:'center',textAlign:'center',paddingTop:66}}>
            <Particles/>
            <div style={{position:'relative',zIndex:5}}>
              <div className="hero-eyebrow" style={{background:'rgba(255,79,163,0.1)',borderColor:'rgba(255,79,163,0.3)',color:'var(--rose)',animation:'none'}}><span>🚨</span><span>24/7 Emergency Response System</span></div>
        ...
        <div className="section">
          ...
          <div className="g2" style={{alignItems:'start',gap:'2.5rem'}}>
            <div>
              ...
              <div className="card" style={{padding:'1.5rem',marginBottom:'1.5rem'}}>
                <div style={{fontFamily:'var(--serif)',fontSize:'0.9rem',fontWeight:700,color:'var(--rose)',marginBottom:'1rem'}}>EMERGENCY STATISTICS</div>
                {[['Avg Response Time','4 minutes'],['Success Rate','98.7%'],['Active Workers','23 online now'],['SOS Handled Today','12 cases']].map(([l,v])=>(<div key={l} style={{display:'flex',justifyContent:'space-between',padding:'0.5rem 0',borderBottom:'1px solid var(--border2)',fontSize:'0.82rem'}}><span style={{color:'var(--text2)'}}>{l}</span><span style={{fontWeight:700,color:'var(--jade)'}}>{v}</span></div>))}
              </div>
            </div>
            <div>
              <div className="sec-title">Nearby Professionals</div>
              {!activeEmrg&&(<div style={{textAlign:'center',padding:'3rem 1.5rem',background:'var(--surface)',border:'1px solid var(--border2)',borderRadius:'var(--r-xl)',backdropFilter:'blur(16px)'}}><div style={{fontSize:'3rem',marginBottom:'1rem'}}>📡</div><div style={{fontFamily:'var(--serif)',color:'var(--text)',marginBottom:'0.5rem'}}>No Active Emergency</div><p style={{fontSize:'0.82rem',color:'var(--text2)'}}>Select an emergency type to activate scanning</p></div>)}
              {activeEmrg&&scanning&&(
                <div style={{textAlign:'center',padding:'2.5rem 1.5rem',background:'rgba(255,79,163,0.05)',border:'1px solid rgba(255,79,163,0.2)',borderRadius:'var(--r-xl)',backdropFilter:'blur(16px)',position:'relative',overflow:'hidden'}}>
                  ...
                  <div style={{fontFamily:'var(--serif)',color:'var(--rose)',letterSpacing:'0.04em',marginBottom:'0.5rem'}}>Scanning for {activeEmrg} help...</div>
                </div>
              )}
              {activeEmrg&&found&&(
                <div>
                  <div style={{background:'rgba(92,224,200,0.08)',border:'1px solid rgba(92,224,200,0.25)',borderRadius:'var(--r-md)',padding:'0.75rem 1rem',marginBottom:'1rem',fontSize:'0.8rem',color:'var(--jade)',display:'flex',gap:'0.5rem',alignItems:'center'}}><span>✅</span> {EXTENDED.filter(w=>w.available).length} professionals found</div>
                  {EXTENDED.map(w=>(<div key={w.name} className="emrg-worker-card card" style={{border:`1px solid ${dispatchedId===w.name?'var(--jade)':'var(--border2)'}`,background:dispatchedId===w.name?'rgba(92,224,200,0.08)':'var(--surface)'}}><img src={w.img} alt={w.name} className="emrg-avatar"/><div style={{flex:1}}><div style={{fontWeight:700,color:'var(--text)',fontSize:'0.88rem'}}>{w.name}</div><div style={{fontSize:'0.72rem',color:'var(--text3)'}}>{w.skill} · ⭐ {w.rating} · {w.jobs} jobs</div><div style={{fontSize:'0.72rem',color:'var(--text2)',marginTop:2}}>Rs. {w.price}/service</div></div><div style={{textAlign:'right'}}><div className="emrg-eta">{w.eta}</div><div style={{fontSize:'0.65rem',color:w.available?'var(--jade)':'#FFAD6B',marginBottom:'0.4rem'}}>{w.available?'🟢 Available':'🟡 Busy'}</div>{dispatchedId===w.name?<span style={{fontSize:'0.7rem',color:'var(--jade)',fontWeight:700}}>✅ Dispatched</span>:<button className="btn btn-sm" style={{background:'linear-gradient(135deg,rgba(255,126,182,0.2),rgba(196,170,255,0.15))',color:'var(--rose)',border:'1px solid rgba(255,126,182,0.35)'}} onClick={()=>{setDispatchedId(w.name);addToast(`🚨 ${w.name} dispatched! ETA ${w.eta}`,'success');}} disabled={!w.available}>{w.available?'Dispatch':'Busy'}</button>}</div></div>))}
          ...
          <div style={{marginTop:'5rem'}}>
            <div className="sec-head"><div className="sec-divider">✦</div><span className="sec-eyebrow">Stay safe</span><h2>Emergency Safety Tips</h2></div>
            <div className="g3">
              {[{ic:'⚡',t:'Electrical Emergency',tips:['Turn off main breaker immediately','Do not touch exposed wires','Keep area dry and clear','Open windows if burning smell']},{ic:'🚿',t:'Water/Plumbing Emergency',tips:['Shut off main water valve','Avoid electrical appliances','Move valuables from flood area','Document damage for insurance']},{ic:'🔥',t:'Gas/Fire Emergency',tips:['Evacuate immediately','Do not use electronics','Call 1122 first','Do not re-enter until cleared']}].map(cat=>(
                <div key={cat.t} className="card" style={{padding:'1.5rem'}}><div style={{fontSize:'2rem',marginBottom:'0.6rem'}}>{cat.ic}</div><div style={{fontFamily:'var(--serif)',fontSize:'0.95rem',fontWeight:700,color:'var(--text)',marginBottom:'0.75rem'}}>{cat.t}</div>{cat.tips.map(tip=>(<div key={tip} style={{display:'flex',gap:'0.5rem',marginBottom:'0.4rem',fontSize:'0.78rem',color:'var(--text2)'}}><span style={{color:'var(--jade)',flexShrink:0}}>→</span>{tip}</div>))}</div>
              ))}
            </div>
          </div>
        </div>
...
const SmartCityPage=()=>{
  ...
  const LIVE_ACTIVITY=[{time:'Just now',msg:'⚡ Ali Khan completed an electrical job in Lahore'},{time:'2 min ago',msg:'🔧 Sara Ahmed booked a plumber in Gujrat'},{time:'4 min ago',msg:'🌟 Fatima Ali got a 5-star review in Islamabad'},{time:'6 min ago',msg:'🚨 Emergency AC repair dispatched in Karachi'},{time:'9 min ago',msg:'✅ Bilal Ahmed completed 50th job milestone'},{time:'12 min ago',msg:'💖 New worker verified: Hina (Beautician, Lahore)'}];
  const SKILL_DIST=SKILLS.map((s,i)=>({skill:s,count:8+i*2+Math.floor(Math.random()*5),pct:20+i*5}));
  return(
    <PageWrapper>
      <div className="smart-page">
        <div style={{background:'linear-gradient(135deg,rgba(92,224,200,0.06),rgba(168,218,255,0.04))',borderBottom:'1px solid var(--border2)',padding:'5rem 2.5rem 3rem',paddingTop:110,textAlign:'center',position:'relative',overflow:'hidden'}}>
          <Particles/>
          <div style={{position:'relative',zIndex:5}}>
            <div className="hero-eyebrow" style={{animation:'none',display:'inline-flex'}}><span style={{display:'inline-block',animation:'pulse2 1.5s ease-in-out infinite'}} className="live-dot"/><span style={{marginLeft:'0.5rem'}}>Live Platform Data</span></div>
            ...
            <p style={{color:'var(--text2)',fontFamily:'var(--serif2)',fontSize:'1.1rem',maxWidth:500,margin:'0 auto'}}>Real-time platform intelligence across all cities and service categories.</p>
          </div>
        </div>
        <div className="section">
          ...
          <div className="g2" style={{alignItems:'start',gap:'2rem',marginBottom:'2rem'}}>
            ...
            <div className="card" style={{padding:'1.5rem'}}>
              ...
              {LIVE_ACTIVITY.map((a,i)=>(<div key={i} style={{display:'flex',gap:'0.7rem',padding:'0.6rem 0',borderBottom:'1px solid var(--border2)',alignItems:'flex-start'}}><div style={{fontSize:'0.68rem',color:'var(--text3)',whiteSpace:'nowrap',marginTop:2,minWidth:65}}>{a.time}</div><div style={{fontSize:'0.78rem',color:'var(--text2)',lineHeight:1.5}}>{a.msg}</div></div>))}
            </div>
          </div>
          ...
          <div className="g2" style={{alignItems:'start',gap:'2rem'}}>
            <div className="card" style={{padding:'1.5rem'}}>
              <div style={{fontFamily:'var(--serif)',fontSize:'1rem',fontWeight:700,color:'var(--text)',marginBottom:'1.2rem'}}>Skill Distribution</div>
              {SKILL_DIST.map(s=>(<div key={s.skill} style={{marginBottom:'0.8rem'}}><div style={{display:'flex',justifyContent:'space-between',fontSize:'0.78rem',marginBottom:4}}><span style={{color:'var(--text)'}}>{s.skill}</span><span style={{color:'var(--lav)',fontWeight:700}}>{s.count} workers</span></div><div className="bar-bg"><div style={{height:'100%',borderRadius:3,background:'linear-gradient(90deg,var(--lav),var(--mist))',width:`${s.pct}%`,transition:'width 0.8s var(--ease)'}}/></div></div>))}
            </div>
            <div>
              <div className="card" style={{padding:'1.5rem',marginBottom:'1rem'}}>
                <div style={{fontFamily:'var(--serif)',fontSize:'1rem',fontWeight:700,color:'var(--text)',marginBottom:'1rem'}}>Platform Health</div>
                {[['Uptime','99.98%','var(--jade)'],['Response Time','142ms','var(--mist)'],['Worker Verification','100%','var(--gold)'],['Booking Success Rate','97.3%','var(--lav)'],['Emergency Response','98.7%','var(--rose)']].map(([l,v,c])=>(<div key={l} style={{display:'flex',justifyContent:'space-between',padding:'0.55rem 0',borderBottom:'1px solid var(--border2)',fontSize:'0.82rem'}}><span style={{color:'var(--text2)'}}>{l}</span><span style={{fontWeight:700,color:c}}>{v}</span></div>))}
              </div>
              <div className="card" style={{padding:'1.5rem'}}>
                <div style={{fontFamily:'var(--serif)',fontSize:'1rem',fontWeight:700,color:'var(--text)',marginBottom:'1rem'}}>Today's Highlights</div>
                {[['New Registrations','12 workers today'],['Jobs Completed','48 this week'],['Emergency Cases','3 resolved'],['Top City','Lahore (28 jobs)'],['Peak Hour','2–5 PM']].map(([l,v])=>(<div key={l} style={{display:'flex',justifyContent:'space-between',padding:'0.55rem 0',borderBottom:'1px solid var(--border2)',fontSize:'0.82rem'}}><span style={{color:'var(--text2)'}}>{l}</span><span style={{fontWeight:700,color:'var(--jade)'}}>{v}</span></div>))}
              </div>
            </div>
          </div>
        </div>
    ...
    <PageWrapper>
      <div className="dash-page">
        ...
        <main className="dash-main">
          ...
          {(tab==='employees'||tab==='workers')&&(<><div style={{marginBottom:'1rem'}}><input className="input" style={{maxWidth:280}} placeholder="🔍 Search…" value={search} onChange={e=>{setSearch(e.target.value);setPg(1);}}/></div><div style={{display:'grid',gridTemplateColumns:'repeat(5,1fr)',gap:'1rem'}}>{(tab==='employees'?paged:WORKERS.slice((pg-1)*PER,pg*PER)).map(e=>(<div key={e.id} className="card" style={{textAlign:'center',padding:'1.2rem 0.7rem'}}><img src={e.avatar} alt={e.name} style={{width:60,height:60,borderRadius:'50%',objectFit:'cover',border:'1.5px solid rgba(92,224,200,0.4)',margin:'0 auto 0.5rem',display:'block',filter:'brightness(0.9)'}}/><div style={{fontWeight:700,fontSize:'0.76rem',color:'var(--text)'}}>{e.name}</div><div style={{fontSize:'0.67rem',color:'var(--jade)',marginTop:2}}>{e.role||e.skill}</div><div style={{color:'var(--gold)',fontSize:'0.65rem',marginTop:3}}>{'⭐'.repeat(Math.floor(e.rating||4.5))}</div></div>))}</div><div className="pagination">{Array.from({length:Math.min(pages,8)},(_,i)=>(<button key={i+1} className={`pbtn ${pg===i+1?'active':''}`} onClick={()=>setPg(i+1)}>{i+1}</button>))}</div></>)}
    ...
    <PageWrapper>
      <div style={{paddingTop:66,minHeight:'100vh'}}>
        <div className="section">
          ...
          <div style={{marginBottom:'2rem',marginTop:'0.5rem'}}><h2 style={{fontFamily:'var(--serif)',fontSize:'2rem',fontWeight:700,letterSpacing:'0.04em',color:'var(--text)'}}>Saved Professionals 💖</h2><p style={{color:'var(--text2)',marginTop:'0.25rem',fontSize:'0.85rem',fontFamily:'var(--serif2)',fontStyle:'italic'}}>{saved.length} worker{saved.length!==1?'s':''} saved</p></div>
      ...
      <div className="about-page">
        <div className="hero" style={{minHeight:'52vh'}}>
          ...
          <div className="hero-content">
            ...
            <h1 className="hero-title">Built with <span className="grad">Purpose</span> for Pakistan</h1>
            <p className="hero-sub">On a mission to make home services safe, reliable, and delightful for every family.</p>
          </div>
        </div>
        <div className="section">
          <div className="g2" style={{gap:'3.5rem',alignItems:'start'}}>
            <div>
              <h2 style={{fontFamily:'var(--serif)',fontSize:'2rem',fontWeight:700,letterSpacing:'0.04em',color:'var(--text)',margin:'0.3rem 0 1.5rem'}}>From a Simple Idea to Pakistan's Most Trusted Platform</h2>
              <div className="tl">
                {[['🌱','2021','The Beginning','Started with 10 workers in Gujrat.'],['🚀','2022','Rapid Growth','Expanded to 5 cities, 500+ verified professionals.'],['💕','2023','Going Nationwide','Reached 10+ cities with 2,000+ monthly bookings.'],['🌊','2024','Servire 2.0','Launched redesigned platform with emergency features.']].map(([ic,y,t,d])=>(<div key={y} className="tl-item"><div className="tl-dot">{ic}</div><div className="tl-content"><div className="tl-year">{y}</div><div className="tl-title">{t}</div><div className="tl-desc">{d}</div></div></div>))}
              </div>
            </div>
            <div className="card" style={{padding:'2.5rem',textAlign:'center'}}>
              <div style={{fontSize:'3.5rem',marginBottom:'1rem'}}>🌊</div>
              <h3 style={{fontFamily:'var(--serif)',fontSize:'1.5rem',fontWeight:700,letterSpacing:'0.04em',color:'var(--text)',marginBottom:'0.75rem'}}>Our Mission</h3>
              <p style={{color:'var(--text2)',lineHeight:1.85,fontSize:'0.9rem',fontFamily:'var(--serif2)'}}>To make finding home services as easy and delightful as any modern digital experience — with verified professionals, transparent pricing, and a warm, caring journey from start to finish.</p>
            </div>
          </div>
        </div>
        <div style={{background:'rgba(92,224,200,0.03)',borderTop:'1px solid var(--border2)',borderBottom:'1px solid var(--border2)',padding:'5rem 2.5rem'}}>
          <div style={{maxWidth:1300,margin:'0 auto'}}>
            <div className="sec-head"><div className="sec-divider">✦</div><h2>Meet the Team</h2></div>
            <div className="team-grid">
              {team.map((name,i)=>(<div key={i} className="team-card card"><img src={`https://randomuser.me/api/portraits/${i%2===0?'women':'men'}/${i+1}.jpg`} alt={name} className="team-av"/><div style={{fontWeight:700,color:'var(--text)',fontSize:'0.86rem'}}>{name}</div><div style={{fontSize:'0.72rem',color:'var(--jade)',marginTop:2}}>{roles[i]}</div></div>))}
            </div>
          </div>
        </div>
        <Footer/>
...
const ContactPage=()=>{
  ...
  const FAQS=[['How do I book a worker?','Browse our directory, view profiles, and click "Book Now". Select date, time, and address to confirm instantly.'],['Are all workers verified?','Yes — every professional undergoes ID verification and background screening before appearing on the platform.'],['Can I cancel a booking?','Cancellations are accepted up to 2 hours before the scheduled time with no charge.'],["What if I'm not satisfied?",'Contact our support within 24 hours of service. We offer a full satisfaction guarantee.'],['Are female workers available?','Yes! We have dedicated female professionals for families who prefer same-gender service providers.'],['How is payment handled?','Payment is made directly to the professional after service. We support cash, JazzCash, and Easypaisa.']];
  return(
    <PageWrapper>
      <div className="contact-page">
        <div className="hero" style={{minHeight:'40vh'}}>
          <div className="hero-city" style={{filter:'brightness(0.45)'}}/>
          <div style={{position:'absolute',inset:0,background:'linear-gradient(135deg,rgba(6,13,24,0.88),rgba(6,13,24,0.72))',zIndex:2}}/>
          <Particles/>
          <div className="hero-content">
            <div className="hero-eyebrow"><span>💬</span><span>We're here for you</span></div>
            <h1 className="hero-title">Get in <span className="grad">Touch</span></h1>
            <p className="hero-sub">Have a question or need support? We reply within 24 hours.</p>
          </div>
        </div>
        <div className="section">
          <div className="g2" style={{gap:'3.5rem'}}>
            <div>
              <div className="sec-title">Send us a message</div>
              {[['name','Your Name','Sara Ahmed','text'],['email','Email','sara@gmail.com','email'],['subject','Subject','How can we help?','text']].map(([k,l,ph,t])=>(<div className="form-group" key={k}><label className="form-label">{l}</label><input className="input" type={t} placeholder={ph} value={form[k]} onChange={e=>setForm(p=>({...p,[k]:e.target.value}))}/></div>))}
              <div className="form-group"><label className="form-label">Message</label><textarea className="input" placeholder="Your message…" value={form.message} onChange={e=>setForm(p=>({...p,message:e.target.value}))}/></div>
              <button className="btn btn-jade btn-lg" style={{justifyContent:'center'}} onClick={()=>{if(form.name&&form.email&&form.message){addToast("Message sent! We'll reply soon 🌊",'success');setForm({name:'',email:'',subject:'',message:''});}else addToast('Please fill all required fields','error');}}>Send Message ✦</button>
            </div>
            <div>
              <div className="sec-title">Contact Info</div>
              {[['📧','Email','hello@servire.pk'],['📱','Phone','+92 300-SERVIRE'],['📍','Office','Satellite Town, Gujrat, Punjab'],['⏰','Hours','Mon–Sat, 9 AM – 8 PM'],['🚨','Emergency','24/7 always available']].map(([ic,l,v])=>(<div key={l} style={{display:'flex',gap:'1rem',marginBottom:'1.4rem',alignItems:'flex-start'}}><span style={{fontSize:'1.4rem'}}>{ic}</span><div><div style={{fontWeight:700,fontSize:'0.83rem',color:'var(--text)'}}>{l}</div><div style={{fontSize:'0.8rem',color:'var(--text2)'}}>{v}</div></div></div>))}
              <div className="sep"/>
              <div className="sec-title">FAQs</div>
              {FAQS.map(([q,a],i)=>(<div key={i} className="faq-item"><div className="faq-q" onClick={()=>setFaqOpen(faqOpen===i?null:i)}>{q}<span style={{color:'var(--jade)'}}>{faqOpen===i?'▲':'▼'}</span></div>{faqOpen===i&&<div className="faq-a">{a}</div>}</div>))}
            </div>
          </div>
        </div>
        <Footer/>
    ...
    <PageWrapper>
      <div className="notfound-page">
        ...
        <div style={{position:'relative',zIndex:5,textAlign:'center'}}>
          ...
          <p className="notfound-desc">The page you're looking for doesn't exist, or may have drifted away on the current.</p>
          <div style={{display:'flex',gap:'0.75rem',justifyContent:'center',marginTop:'1.5rem',flexWrap:'wrap'}}>
            <button className="btn btn-jade btn-lg" onClick={()=>navigate('/')}>🏠 Back to Home</button>
            <button className="btn btn-ghost btn-lg" onClick={()=>navigate('/workers')}>🔍 Browse Workers</button>
          </div>
        </div>
      </div>
    </PageWrapper>
...
// ⚠️ YEH LINE SABSE IMPORTANT HAI - LAST LINE MEIN YAHI HONA CHAHIYE
export default App;
